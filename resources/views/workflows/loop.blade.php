    ...
    @foreach ($collection as  $ky =>  $item)
        ...
        <tr>
            ...
            <td>
                ...
                @if($res)
                    <span class="label label-lg label-light-success label-inline _change_active1" data-id="">Yes</span>
                @else
                    <span class="label label-lg label-light-danger label-inline _change_active1" data-id="">No</span>
                @endif
            </td>
            @can('Active Workflow')
                <td>
                    @if($item->active)
                        <span class="label label-lg label-light-success label-inline _change_active"
                              data-id="{{ $item->id }}">Active</span>
                    @else
                        <span class="label label-lg label-light-danger label-inline _change_active"
                              data-id="{{ $item->id }}">Inactive</span>
                    @endif
                </td>
            @endcan
            @can('Edit Workflow')
                <td>
                    <a href='{{url("$route")}}/{{ $item->id }}' class="btn btn-sm btn-clean btn-icon mr-2"
                       title="Edit Workflow"> <span class="svg-icon svg-icon-md">	                                <svg
