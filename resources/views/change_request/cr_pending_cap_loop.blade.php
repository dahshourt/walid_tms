...
@if ($collection)
    ...
    @foreach ($collection as $item)
        ...
        @if (
            (!$view_technical_team_flag && $isPendingCab) ||
                ($view_technical_team_flag && in_array($default_group, $assigned_technical_teams) && $check_if_status_active))
            <tr>
                @can('Edit CR_Pending')
                ...
                <th scope="row">
                    <span class="description-preview text-primary" data-description="{{ e($item->description) }}"
                        role="button">
                        {{ \Illuminate\Support\Str::limit($item->description, 50) }}
                    </span>
                </th>
                <td>{{ $item->getCurrentStatus() && $item->getCurrentStatus()->status ? $item->getCurrentStatus()->status->status_name : '' }}
                </td>
                    ...
                    <td>
                        <div class="d-inline-flex gap-2">
                            <button type="button" class="btn btn-outline-success btn-sm _approved_active_cab"
                                data-id="{{ $item->id }}" data-token="{{ $token }}">
                                ✅ Approved
                            </button>
