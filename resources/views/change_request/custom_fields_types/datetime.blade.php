...
<div class="col-md-6 change-request-form-field field_{{$item->CustomField->name}}">
    ...
    @endif
        @if(isset($cr))
        <input type="datetime-local" name="{{ $item->CustomField->name }}" value="{{ $custom_field_value }}" class="form-control form-control-lg {{ $item->CustomField->class }}" {{ $required }} {{ $disabled }} />
        @else
            @if(isset($cr))
