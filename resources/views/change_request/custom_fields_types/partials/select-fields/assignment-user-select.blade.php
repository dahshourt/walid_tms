...
{{-- partials/select-fields/assignment-user-select.blade.php --}}
<select name="{{ $item->CustomField->name }}" class="form-control form-control-lg"
    ...
    @foreach($assignment_users as $assignment_user)
        <option value="{{ $assignment_user->id }}" {{ old($assignment_user->user_name, $custom_field_value) == $assignment_user->id ? 'selected' : '' }}>
