...
@foreach ($collection as $item)
    <tr>
        ...
        <td>{{ $item->name }}</td>
        @can('Active Requester Department')
        <td>
            @if($item->active)
        ...
        @endcan
        @can('Edit Requester Department')
        <td>
            <a href='{{url("$route")}}/{{ $item->id }}/edit' class="btn btn-sm btn-clean btn-icon mr-2" title="Edit Requester Department">
