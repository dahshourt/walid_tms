        ...
        if(isset($request->custom_field_id))
        {
            ...
            foreach($request->custom_field_id as $key=>$value)
            {
                if(isset($value) && $value != false){
                    ...
                    if($request->status_id) $data['status_id'] =  $request->status_id;
                    if($request->group_id) $data['group_id'] =  $request->group_id;
                    if($request->wf_type_id) $data['wf_type_id'] =  $request->wf_type_id;
                    if(isset($request->sort[$key])) $data['sort'] =  $request->sort[$key];
                    if(isset($request->validation_type_id[$key])) $data['validation_type_id'] =  $request->validation_type_id[$key];
                    if(isset($request->enable[$key])) $data['enable'] = $request->enable[$key];
