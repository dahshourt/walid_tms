...
class ChangeRequestRepository implements ChangeRequestRepositoryInterface
{
    ...
    public function create(array $data): array
    {
        ...
        // $this->StoreLog($change_request->id, $request,$this->changeRequest_old, 'create');
    ...
    public function isStartTimeInFuture($startTime)
    {
    ...
    public function update($id, $request)
    {
        ...
        $workflow_tech_team = NewWorkFlow::find($new_status_id);

        $old_status_id = null;
        if ($request->old_status_id) {
        ...
        if ($old_status_data->view_technical_team_flag) {
            ...
            if ($old_status_data->id == 127) {

                $TechnicalCr->technical_cr_team()->where('group_id', $technical_default_group)->update([
                    'status' => '1',
                ]);
                TechnicalCrTeam::create([
                    'group_id' => $technical_default_group,
                    'technical_cr_id' => $TechnicalCr->id,
                    'current_status_id' => $workflow_tech_team->workflowstatus[0]->to_status_id,
                    'status' => '0',
                ]);
            } elseif ($old_status_data->id == 128) {
                $TechnicalCr->technical_cr_team()->where('group_id', $technical_default_group)->update([
                    'status' => '1',
                ]);
        ...
        // $request['assignment_user_id'] = $user->id;
        if ($new_status_id) {
            $request['new_status_id'] = $new_status_id;
        }
        if ($old_status_id) {
            $request['old_status_id'] = $old_status_id;
        }
        ...
        if (isset($request->cr_member)) {
            Change_request_statuse::where('cr_id', $id)->where('new_status_id', $request->old_status_id)->where('active', '1')->update(['assignment_user_id' => $request->cr_member]);
        }

        if (isset($request->rtm_member)) {
            Change_request_statuse::where('cr_id', $id)->where('new_status_id', $request->old_status_id)->where('active', '1')->update(['assignment_user_id' => $request->rtm_member]);
        }
        if (isset($request->tester_id)) {
            Change_request_statuse::where('cr_id', $id)->where('new_status_id', $request->old_status_id)->where('active', '1')->update(['assignment_user_id' => $request->tester_id]);
        }
        if (isset($request->developer_id)) {
            Change_request_statuse::where('cr_id', $id)->where('new_status_id', $request->old_status_id)->where('active', '1')->update(['assignment_user_id' => $request->developer_id]);
        }
        if (isset($request->designer_id)) {
            Change_request_statuse::where('cr_id', $id)->where('new_status_id', $request->old_status_id)->where('active', '1')->update(['assignment_user_id' => $request->designer_id]);
        }
    ...
    public function UpateChangeRequestStatus($id, $request)
    {
        ...
        // dd($new_workflow_id);
        if ($new_workflow_id) {
            $workflow = NewWorkFlow::find($new_workflow_id);
        } else {
        ...
        if ($workflow) {
            ...
            if ($TechnicalCr) {
                $count_all_technical_teams_per_status = $TechnicalCr->technical_cr_team()->where('current_status_id', $old_status_id)->count(); // get count for all users that are approve CR
                $count_approved_technical_teams_per_status = $TechnicalCr->technical_cr_team()->where('current_status_id', $old_status_id)->where('status', '1')->count();
            }
            ...
            // dd($id, $request,$workflow_active,$old_status_id);
            ...
            if ($old_status_id != 127) {
                $cr_status->save();
            } elseif ($old_status_id == 127 && $count_all_technical_teams_per_status == $count_approved_technical_teams_per_status) {
                $cr_status->save();
            }
            ...
            foreach ($workflow->workflowstatus as $key => $item) {
                ...
                if ($item->dependency_ids) {
                    ...
                    $to_remove = [$item->new_workflow_id];
    ...
    public function getAllWithoutPagination($group = null)
    {
        ...
        if ($group_applications) {
            $changeRequests = $changeRequests->whereIn('application_id', $group_applications);
        }
    ...
    public function getViewStatuses($group = null, $id = null)
    {
        ...
        // Fetch and return the statuses related to the group(s)
        $view_statuses = $view_statuses->groupBy('status_id')->get()->pluck('status_id')->toArray();
        $technical_cr_team_status = $this->GetTechnicalTeamCurrentStatus($id);
        if ($technical_cr_team_status) {
            ...
            // dd($view_statuses,$technical_cr_team_status->current_status_id);
        }
    ...
    public function my_assignments_crs()
    {
        ...
        $crs = Change_request::with('Req_status.status')
            ->whereHas('Req_status', function ($query) use ($user_id) {
                $query->where('assignment_user_id', $user_id);

            })
            ->whereHas('Req_status', function ($query) use ($view_statuses) {
                $query->whereIn('new_status_id', $view_statuses);
            })
    ...
    protected function handleCustomFields(int $crId, array $data): void
    {
        // dd($data);
        foreach ($data as $key => $value) {
            if ($key !== '_token' && $key !== 'business_attachments' && $key !== 'technical_attachments') {
